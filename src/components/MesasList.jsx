...
function MesasList({ userData, onSelectMesa, onVerStatus, onAdmin, onFecharMesa, onGerenciarEquipe, onLogout }) {
  const [mesas, setMesas] = useState([]);function MesasList({ userData, onSelectMesa, onVerStatus, onAdmin, onFecharMesa, onGerenciarEquipe, onDashboard, onLogout }) {
 ...
 const carregarPedidosAtivos = async () => {
  const { data, error } = await supabase
    ...
    .eq('garcom_id', userData.id) // APENAS PEDIDOS DO GARÇOM
    .in('status', ['em_preparo', 'pronto']); // APENAS EM PREPARO E PRONTO
...
<div style={{ marginBottom: '20px', textAlign: 'center', display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap' }}>
  {userData.tipo === 'admin' && (
    <button 
      ...
      style={{ padding: '12px 30px', fontSize: '14px', background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', color: 'white' }}
    >
      📊 Dashboard
    </button>
  )}
      ...
      <div className="mesas-grid">
        {mesas.map((mesa) => (
          <div key={mesa.id} style={{ position: 'relative' }}>
            ...
            {mesa.status !== 'livre' && mesa.status === 'ocupada' && (
  <button
    onClick={(e) => {
      e.stopPropagation();
      onFecharMesa(mesa);
